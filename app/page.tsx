...
import { FaShoppingCart, FaMapMarkerAlt, FaClock, FaTimes, FaPlus, FaMinus, FaTrash, FaSearch, FaFrown, FaArrowUp } from 'react-icons/fa';
  ...
  return (
    ...
    <main>
      <header className="fixed-top w-100" style={{ zIndex: 1000 }}>
         {/* Top Bar */}
         <div className="bg-dark text-light py-1 px-4 d-none d-md-flex justify-content-between align-items-center shadow-sm" style={{ fontSize: '0.85rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="d-flex gap-4 fw-bold opacity-75">
               <span className="d-flex align-items-center gap-2">
                  <motion.svg animate={{ rotate: [0, -15, 15, 0] }} transition={{ repeat: Infinity, duration: 2 }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--bs-primary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></motion.svg>
                  10:00 AM - 04:00 AM
               </span>
               <span className="d-flex align-items-center gap-2">
                  <motion.svg animate={{ y: [0, -4, 0] }} transition={{ repeat: Infinity, duration: 1.5 }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--bs-primary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></motion.svg>
                  Kamra Road, Attock
               </span>
            </div>
            <div className="fw-bold text-white text-uppercase" style={{ letterSpacing: '1px' }}>
               <motion.span animate={{ opacity: [1, 0.4, 1] }} transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}>🚀</motion.span> <span className="text-ember">Attock's Finest</span> Fast Food
            </div>
         </div>
         ...
         >
           <div className="container px-3 px-md-4 d-flex align-items-center justify-content-between position-relative">
             <a className="navbar-brand font-bebas fs-2 m-0 d-flex align-items-center gap-2" href="#" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
               <div className="bg-ember text-white rounded-circle d-flex align-items-center justify-content-center" style={{ width: '40px', height: '40px', fontSize: '1.2rem' }}>🍔</div>
               <span className="fw-bold tracking-wide d-none d-sm-block" style={{ letterSpacing: '1px' }}>
             ...
             <div className="d-flex align-items-center gap-2 gap-md-3">
               ...
               <motion.div 
                  className="d-flex align-items-center bg-dark rounded-pill border overflow-hidden"
                  animate={{ 
                     width: isSearchExpanded ? (window.innerWidth < 768 ? '180px' : '300px') : '42px',
                     borderColor: isSearchFocused ? 'rgba(228, 0, 43, 0.5)' : 'rgba(255,255,255,0.1)'
                  }}
                  style={{ height: '42px', transition: 'border-color 0.3s' }}
               >
                  <div 
               ...
               <motion.button 
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="btn btn-ember fw-bold d-flex align-items-center gap-2 rounded-pill px-3 px-md-4 py-2 shadow-lg" 
                  style={{ border: '2px solid transparent' }} 
                  onClick={() => setCartOpen(true)}
               >
                 <FaShoppingCart className="fs-5" /> 
                 <span className="d-none d-sm-inline fs-5 font-bebas tracking-wide" style={{ letterSpacing: '1px' }}>BASKET</span>
                 <span className="badge rounded-circle ms-1 px-2 py-1 bg-white text-ember shadow-sm" style={{ fontSize: '0.9rem', transform: 'translateY(-1px)' }}>
      ...
      <section className="hero text-center position-relative" style={{ paddingTop: '180px', paddingBottom: '80px', background: 'var(--bs-body-bg)' }}>
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="position-relative" style={{ zIndex: 1 }}>
          <h1 className="font-bebas display-1 fw-bold text-white mb-0" style={{ fontSize: 'clamp(4rem, 12vw, 9rem)', lineHeight: '0.9', letterSpacing: '2px' }}>
            <span className="text-ember">TURBO</span> BITES
          </h1>
          <p className="text-white fs-3 fw-bold text-uppercase mb-5 mt-4" style={{ letterSpacing: '5px' }}>Fast Delivery</p>

          <div className="mx-auto mt-4 px-3 position-relative" style={{ maxWidth: '600px', zIndex: 50 }}>
             ...
             <AnimatePresence>
               {isSearchFocused && searchQuery && (
                 <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }} 
                    exit={{ opacity: 0, y: -10 }}
                    className="position-absolute w-100 mt-2 px-3 start-0"
                 >
                    <div className="solid-panel rounded-4 shadow-lg overflow-hidden border" style={{ borderColor: 'rgba(255,255,255,0.1)', maxHeight: '300px', overflowY: 'auto' }}>
                       {allItems
                          .filter((item: any) => 
                             item.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                             (item.desc && item.desc.toLowerCase().includes(searchQuery.toLowerCase()))
      ...
      <section className="container py-5 mt-4" ref={menuRef}>
          ...
          <>
            ...
            >
              <div className="p-4 border-bottom d-flex justify-content-between align-items-center bg-white">
                <h3 className="font-bebas fs-2 text-dark mb-0 d-flex align-items-center gap-2"><FaShoppingCart className="text-ember" /> Your Basket</h3>
              ...
              <div className="flex-grow-1 overflow-auto p-4 d-flex flex-column gap-3 bg-light">
                {cart.length === 0 ? (
              ...
              <div className="p-4 bg-white border-top shadow-lg" style={{ zIndex: 10 }}>
                 ...
                 <div className="d-flex flex-column gap-3 mb-4">
                    <input type="text" className="form-control border-0 py-3 rounded-3 text-dark fw-bold" style={{ background: '#f8f9fa', boxShadow: 'inset 0 0 0 1px #e9ecef' }} placeholder="Your Name *" value={customer.name} onChange={e => setCustomer({...customer, name: e.target.value})} />
                    <input type="tel" className="form-control border-0 py-3 rounded-3 text-dark fw-bold" style={{ background: '#f8f9fa', boxShadow: 'inset 0 0 0 1px #e9ecef' }} placeholder="Phone Number *" value={customer.phone} onChange={e => setCustomer({...customer, phone: e.target.value})} />
                    <textarea className="form-control border-0 py-3 rounded-3 text-dark fw-bold" style={{ background: '#f8f9fa', boxShadow: 'inset 0 0 0 1px #e9ecef' }} rows={2} placeholder="Delivery Details / Street Address *" value={customer.address} onChange={e => setCustomer({...customer, address: e.target.value})}></textarea>
                 </div>
